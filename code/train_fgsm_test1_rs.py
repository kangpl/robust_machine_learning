...
def eval(args, model, testloader, criterion, finaleval=False):
    ...
    test_test1_loss, test_test1_correct, test_cos_grad, test_cos_delta = 0, 0, 0, 0
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        zero = torch.zeros_like(inputs).to(args.device)
        zero.requires_grad = True
        clean_outputs = model(inputs + zero)
        clean_loss = F.cross_entropy(clean_outputs, targets)
        clean_loss.backward()
        grad_input = zero.grad.detach()
        grad_input_sign = torch.sign(grad_input)
        grad_input_sign_norm = grad_input_sign.view(grad_input_sign.shape[0], -1).norm(dim=1)
        ...
        gard_random_sign = torch.sign(grad_random)
        grad_random_sign_norm = gard_random_sign.view(gard_random_sign.shape[0], -1).norm(dim=1)

        if args.clamp:
            fgsm_delta = clamp(delta + args.train_fgsm_ratio * args.epsilon * torch.sign(grad_random), -args.epsilon,
                               args.epsilon)
            fgsm_test1_delta = clamp(delta + args.train_fgsm_ratio * args.epsilon * torch.sign(grad_input), -args.epsilon,
                               args.epsilon)
        else:
            fgsm_delta = delta + args.train_fgsm_ratio * args.epsilon * torch.sign(grad_random)
            fgsm_test1_delta = delta + args.train_fgsm_ratio * args.epsilon * torch.sign(grad_input)
        fgsm_delta = clamp(fgsm_delta, lower_limit - inputs, upper_limit - inputs).detach()
        fgsm_delta_norm = fgsm_delta.view(fgsm_delta.shape[0], -1).norm(dim=1)

        fgsm_test1_delta = clamp(fgsm_test1_delta, lower_limit - inputs, upper_limit - inputs).detach()
        fgsm_test1_delta_norm = fgsm_test1_delta.view(fgsm_test1_delta.shape[0], -1).norm(dim=1)


        fgsm_outputs = model(inputs + fgsm_delta)
        fgsm_loss = F.cross_entropy(fgsm_outputs, targets)
        fgsm_test1_outputs = model(inputs + fgsm_test1_delta)
        fgsm_test1_loss = F.cross_entropy(fgsm_test1_outputs, targets)
        ...
        cos_grad = cal_cos_similarity(grad_input_sign, gard_random_sign, grad_input_sign_norm, grad_random_sign_norm)
        cos_delta = cal_cos_similarity(fgsm_delta, fgsm_test1_delta, fgsm_delta_norm, fgsm_test1_delta_norm)
        ...
        test_test1_loss += fgsm_test1_loss.item() * targets.size(0)
        test_test1_correct += (fgsm_test1_outputs.max(dim=1)[1] == targets).sum().item()
        test_cos_grad += cos_grad.sum().item()
        test_cos_delta += cos_delta.sum().item()
    ...
    return test_clean_loss / test_total, 100. * test_clean_correct / test_total, \
           ...
           test_test1_loss / test_total, 100. * test_test1_correct / test_total, test_cos_grad / test_total, test_cos_delta / test_total


def tb_writer(writer, epoch, lr, train_clean_loss, train_clean_acc, train_fgsm_loss, train_fgsm_acc, train_fgsm_delta_norm,
              test_clean_loss, test_clean_acc, test_fgsm_loss, test_fgsm_acc, test_fgsm_delta_norm, test_pgd10_loss, test_pgd10_acc, test_pgd10_delta_norm,
              test_test1_loss, test_test1_acc, test_cos_grad, test_cos_delta):
    writer.add_scalars('loss',
                       {'train_clean': train_clean_loss, 'train_fgsm': train_fgsm_loss,
                        'test_clean': test_clean_loss, 'test_fgsm': test_fgsm_loss,
                        'test_pgd10': test_pgd10_loss, 'test_test1': test_test1_loss}, epoch)
    writer.add_scalars('accuracy',
                       {'train_clean': train_clean_acc, 'train_fgsm': train_fgsm_acc,
                        'test_clean': test_clean_acc, 'test_fgsm': test_fgsm_acc,
                        'test_pgd10': test_pgd10_acc, 'test_test1': test_test1_acc}, epoch)
    writer.add_scalar('learning rate', lr, epoch)
    writer.add_scalars('cosine similarity', {'test_grad_sign': test_cos_grad, 'test_delta': test_cos_delta}, epoch)
    writer.add_scalars('delta_norm', {'train_fgsm': train_fgsm_delta_norm, 'test_fgsm': test_fgsm_delta_norm, 'test_pgd10': test_pgd10_delta_norm}, epoch)
...
def eval_init(args, writer, logger, model, trainloader, testloader, criterion, opt):
    ...
    test_clean_loss, test_clean_acc, test_fgsm_loss, test_fgsm_acc, test_fgsm_delta_norm, test_pgd_loss, test_pgd_acc, test_pgd10_delta_norm, test_test1_loss, test_test1_acc, test_cos_grad, test_cos_delta = eval(args, model, testloader, criterion)
    tb_writer(writer, 0, opt.param_groups[0]['lr'],
              ...
              test_clean_loss, test_clean_acc, test_fgsm_loss, test_fgsm_acc, test_fgsm_delta_norm, test_pgd_loss, test_pgd_acc, test_pgd10_delta_norm, test_test1_loss, test_test1_acc, test_cos_grad, test_cos_delta)
    logger.info(
