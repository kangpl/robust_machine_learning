...
def train(args, model, trainloader, optimizer, criterion, step_lr_scheduler):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        inputs_grad, clean_outputs, clean_loss = get_input_grad_v2(model, inputs, targets)
        norm = get_batch_l2_norm(inputs_grad)
        if batch_idx == 0:
            prev_norm = norm.cpu().numpy().mean()
        elif args.adjust_fgsm_alpha and norm.cpu().numpy().mean() / prev_norm > 5:
            args.train_fgsm_ratio /= 2
            prev_norm = norm.cpu().numpy().mean()
