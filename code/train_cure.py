...
def main():
    ...
    h = np.linspace(0.1, args.cure_h_max, 5)
    h_all = args.num_epochs * [args.cure_h_max]
    h_all[:len(h)] = h
    for epoch in range(start_epoch, args.num_epochs):
        lr = 8e-4 - (8e-4 - 1e-5) / args.num_epochs * epoch
        # lr = 8e-4
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr

        start_time = time.time()
        train_loss, train_acc, train_reg = train(args, model, trainloader, optimizer, criterion, h_all[epoch])
