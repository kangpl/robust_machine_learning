...
def train_batch(args, model, inputs, targets, optimizer, criterion):
    ...
    fgsm_loss.backward()
    optimizer.step()
    return fgsm_loss, fgsm_outputs, fgsm_delta_norm


# Training
def train_before_frozen(args, model, trainloader, optimizer, criterion, step_lr_scheduler):
    model.train()
    train_fgsm_loss, train_fgsm_correct, train_fgsm_delta_norm, train_total = 0, 0, 0, 0
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        inputs, targets = inputs.to(args.device), targets.to(args.device)
        fgsm_loss, fgsm_outputs, fgsm_delta_norm = train_batch(args, model, inputs, targets, optimizer, criterion)
    ...
    return train_fgsm_loss / train_total, 100. * train_fgsm_correct / train_total, train_fgsm_delta_norm / train_total
...
def train_at_frozen(args, model, trainloader, optimizer, criterion, step_lr_scheduler):
    ...
    np.save(f, np.concatenate(perturbed_inputs, axis=0))
    f = open(os.path.join(args.PERTURBED_DIR, args.exp_name+'_targets.npy'), 'wb')
    np.save(f, np.concatenate(perturbed_targets, axis=0))
    return train_fgsm_loss / train_total, 100. * train_fgsm_correct / train_total, train_fgsm_delta_norm / train_total


def train_after_frozen(args, model, trainloader, optimizer, criterion, step_lr_scheduler):
    model.train()
    train_fgsm_loss, train_fgsm_correct, train_total = 0, 0, 0
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        inputs, targets = inputs.to(args.device), targets.to(args.device)

        fgsm_outputs = model(inputs)
        fgsm_loss = criterion(fgsm_outputs, targets)
        optimizer.zero_grad()
        fgsm_loss.backward()
        optimizer.step()
        ...
        return train_fgsm_loss / train_total, 100. * train_fgsm_correct / train_total, -1
...
def tb_writer(writer, epoch, lr, train_fgsm_loss, train_fgsm_acc, train_fgsm_delta_norm,
...
def main():
    ...
    for epoch in range(args.num_epochs):
        if epoch+1 == args.frozen_epoch+1:
            trainset = Cifar10Dataset(dataPath=os.path.join(args.PERTURBED_DIR, args.exp_name+'_inputs.npy'), labelPath=os.path.join(args.PERTURBED_DIR, args.exp_name+'_targets.npy'))
            trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, shuffle=True, num_workers=2)

        start_time = time.time()
        if epoch+1 < args.frozen_epoch:
            train_fgsm_loss, train_fgsm_acc, train_fgsm_delta_norm = train_before_frozen(args, model, trainloader, optimizer, criterion, step_lr_scheduler)
        elif epoch+1 == args.frozen_epoch:
            train_fgsm_loss, train_fgsm_acc, train_fgsm_delta_norm = train_at_frozen(args, model, trainloader, optimizer, criterion, step_lr_scheduler)
        else:
            train_fgsm_loss, train_fgsm_acc, train_fgsm_delta_norm = train_after_frozen(args, model, trainloader, optimizer, criterion, step_lr_scheduler)
        ...
        tb_writer(writer, epoch+1, optimizer.param_groups[0]['lr'],
                  train_fgsm_loss, train_fgsm_acc, train_fgsm_delta_norm,
                  test_clean_loss, test_clean_acc, test_fgsm_loss, test_fgsm_acc, test_fgsm_delta_norm, test_pgd_loss, test_pgd_acc, test_pgd10_delta_norm, test_input_grad_norm, test_perturbed_grad_norm, test_df50_loop, test_df50_perturbation_norm, test_cos)
        logger.info(
            '%d \t %.1f \t \t %.1f \t \t %.4f \t %.4f \t %.2f \t \t %.4f \t \t %.2f \t \t \t %.4f \t \t %.2f \t %.2f \t %.2f',
