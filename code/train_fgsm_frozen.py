...
def get_args():
    ...
    parser.add_argument('--frozen_epoch', default=10, type=int)
...
def calculate_fgsm_delta(args, model, inputs, targets):
...
# Training
def train_before_frozen(args, model, trainloader, optimizer, criterion, step_lr_scheduler):
    model.train()
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        fgsm_delta, fgsm_delta_norm = calculate_fgsm_delta(args, model, inputs, targets)
...
def train_at_frozen(args, model, trainloader, optimizer, criterion, step_lr_scheduler):
    model.train()
    train_clean_loss, train_clean_correct, train_fgsm_loss, train_fgsm_correct, train_fgsm_delta_norm, train_total = 0, 0, 0, 0, 0, 0
    clean_inputs, perturbations, clean_targets = [], [], []
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        inputs, targets = inputs.to(args.device), targets.to(args.device)
        fgsm_delta, fgsm_delta_norm = calculate_fgsm_delta(args, model, inputs, targets)
        fgsm_outputs = model(inputs + fgsm_delta)
        ...
        train_fgsm_correct += (fgsm_outputs.max(dim=1)[1] == targets).sum().item()
        train_fgsm_delta_norm += fgsm_delta_norm.sum().item()
        clean_inputs.append(inputs.cpu().numpy())
        perturbations.append(fgsm_delta.cpu().numpy())
        clean_targets.append(targets.cpu().numpy())
        train_total += targets.size(0)
        if args.lr_schedule == 'cyclic':
            step_lr_scheduler.step()
    f = open(os.path.join(args.PERTURBED_DIR, args.exp_name+'_inputs.npy'), 'wb')
    np.save(f, np.concatenate(clean_inputs, axis=0))
    f = open(os.path.join(args.PERTURBED_DIR, args.exp_name + '_perturbations.npy'), 'wb')
    np.save(f, np.concatenate(perturbations, axis=0))
    f = open(os.path.join(args.PERTURBED_DIR, args.exp_name+'_targets.npy'), 'wb')
    np.save(f, np.concatenate(clean_targets, axis=0))
    return train_clean_loss / train_total, 100 * train_clean_correct / train_total, \
           train_fgsm_loss / train_total, 100. * train_fgsm_correct / train_total, train_fgsm_delta_norm / train_total


def train_after_frozen(args, model, trainloader, optimizer, criterion, step_lr_scheduler):
    model.train()
    train_clean_loss, train_clean_correct, train_fgsm_loss, train_fgsm_correct, train_fgsm_delta_norm, train_total = 0, 0, 0, 0, 0, 0
    for batch_idx, (inputs, fgsm_delta, targets) in enumerate(trainloader):
        inputs, fgsm_delta, targets = inputs.to(args.device), fgsm_delta.to(args.device), targets.to(args.device)
        if args.add_random:
            delta = torch.zeros_like(inputs).to(args.device)
            for i in range(len(args.epsilon)):
                delta[:, i, :, :].uniform_(-args.epsilon[i][0][0].item(), args.epsilon[i][0][0].item())
            # fgsm_delta = clamp(fgsm_delta + delta, -args.epsilon, args.epsilon)
            fgsm_delta = clamp(fgsm_delta + delta, lower_limit - inputs, upper_limit - inputs)
        fgsm_delta_norm = fgsm_delta.view(fgsm_delta.shape[0], -1).norm(dim=1)
        fgsm_outputs = model(inputs + fgsm_delta)
        fgsm_loss = criterion(fgsm_outputs, targets)
...
def eval(args, model, testloader, criterion, finaleval=False):
    ...
    test_input_grad_norm, test_df50_loop, test_df50_perturbation_norm, test_fgsm_pgd10_cos = 0, 0, 0, 0
    ...
    return test_clean_loss / test_total, 100. * test_clean_correct / test_total, \
           ...
           test_input_grad_norm / test_total, test_df50_loop / test_total, test_df50_perturbation_norm / test_total, test_fgsm_pgd10_cos / test_total


def tb_writer(writer, epoch, lr, train_clean_loss, train_clean_acc, train_fgsm_loss, train_fgsm_acc, train_fgsm_delta_norm,
              test_clean_loss, test_clean_acc, test_fgsm_loss, test_fgsm_acc, test_fgsm_delta_norm, test_pgd10_loss, test_pgd10_acc, test_pgd10_delta_norm, test_input_grad_norm, test_df50_loop, test_df50_perturbation_norm, test_fgsm_pgd10_cos):
...
def eval_init(args, writer, logger, model, trainloader, testloader, criterion, opt):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        train_fgsm_delta_norm += fgsm_delta_norm.sum().item()
        train_total += targets.size(0)
    test_clean_loss, test_clean_acc, test_fgsm_loss, test_fgsm_acc, test_fgsm_delta_norm, test_pgd_loss, test_pgd_acc, test_pgd10_delta_norm, test_input_grad_norm, test_df50_loop, test_df50_perturbation_norm, test_cos = eval(args, model, testloader, criterion)
    tb_writer(writer, 0, opt.param_groups[0]['lr'],
              ...
              test_clean_loss, test_clean_acc, test_fgsm_loss, test_fgsm_acc, test_fgsm_delta_norm, test_pgd_loss, test_pgd_acc, test_pgd10_delta_norm, test_input_grad_norm, test_df50_loop, test_df50_perturbation_norm, test_cos)
    logger.info(
...
def main():
    ...
    args.LOG_DIR = './output/log'
    args.TENSORBOARD_DIR = './output/tensorboard'
    args.CHECKPOINT_DIR = '../../../../scratch/pekang/checkpoint/'
    args.PERTURBED_DIR = '../../../../scratch/pekang/perturbed/'
    ...
    if not os.path.exists(args.PERTURBED_DIR):
        os.mkdir(args.PERTURBED_DIR)
    ...
    logging.basicConfig(format='[%(asctime)s] - %(message)s',
                        datefmt='%Y/%m/%d %H:%M:%S',
                        level=logging.INFO,
                        filename=os.path.join(args.LOG_DIR, args.exp_name))
    # Model
    args.device = 'cpu'
    ...
    if args.finetune:
        ...
        assert os.path.isfile(os.path.join(args.CHECKPOINT_DIR,
                                           args.resumed_model_name)), f'Error: no asked checkpoint file {args.resumed_model_name} found! '
        checkpoint = torch.load(os.path.join(args.CHECKPOINT_DIR, args.resumed_model_name))
    ...
    for epoch in range(args.num_epochs):
        if epoch+1 == args.frozen_epoch+1:
            trainsetFrozen = Cifar10Dataset(dataPath=os.path.join(args.PERTURBED_DIR, args.exp_name+'_inputs.npy'), perturbationPath=os.path.join(args.PERTURBED_DIR, args.exp_name+'_perturbations.npy'), labelPath=os.path.join(args.PERTURBED_DIR, args.exp_name+'_targets.npy'))
            trainloaderFrozen = torch.utils.data.DataLoader(trainsetFrozen, batch_size=args.batch_size, shuffle=True, num_workers=2)

        start_time = time.time()
        if epoch+1 < args.frozen_epoch:
            train_clean_loss, train_clean_acc, train_fgsm_loss, train_fgsm_acc, train_fgsm_delta_norm = train_before_frozen(args, model, trainloader, optimizer, criterion, step_lr_scheduler)
        elif epoch+1 == args.frozen_epoch:
            train_clean_loss, train_clean_acc, train_fgsm_loss, train_fgsm_acc, train_fgsm_delta_norm = train_at_frozen(args, model, trainloader, optimizer, criterion, step_lr_scheduler)
        else:
            train_clean_loss, train_clean_acc, train_fgsm_loss, train_fgsm_acc, train_fgsm_delta_norm = train_after_frozen(args, model, trainloaderFrozen, optimizer, criterion, step_lr_scheduler)
        ...
        if args.lr_schedule == 'multistep':
            step_lr_scheduler.step()
        if epoch+1 == args.frozen_epoch-1 or epoch+1 == args.frozen_epoch or epoch+1 == args.frozen_epoch+1 or epoch+1 == args.frozen_epoch+2 or epoch+1 == args.frozen_epoch+30:
            save_checkpoint(model, epoch + 1, train_fgsm_loss, train_fgsm_acc, test_clean_loss, test_clean_acc,
                            test_pgd_loss, test_pgd_acc, os.path.join(args.CHECKPOINT_DIR, args.exp_name + f'_{epoch+1}.pth'))
        if test_pgd_acc >= best_test_pgd_acc:
            save_checkpoint(model, epoch+1, train_fgsm_loss, train_fgsm_acc, test_clean_loss, test_clean_acc,
                            test_pgd_loss, test_pgd_acc, os.path.join(args.CHECKPOINT_DIR, args.exp_name + f'_best.pth'))
    ...
    save_checkpoint(model, epoch+1, train_fgsm_loss, train_fgsm_acc, test_clean_loss, test_clean_acc, test_pgd_loss,
                    test_pgd_acc, os.path.join(args.CHECKPOINT_DIR, args.exp_name + f'_final.pth'))

    # Evaluate best and final
    logger.info('best')
    checkpoint = torch.load(os.path.join(args.CHECKPOINT_DIR, args.exp_name + f'_best.pth'))
    model.load_state_dict(checkpoint['model'])
    best_clean_loss, best_clean_acc, _, _, _, best_pgd_loss, best_pgd_acc, best_pgd_delta_norm, _, _, _, _ = eval(args, model, testloader, criterion, finaleval=True)
    ...
    logger.info('final')
    checkpoint = torch.load(os.path.join(args.CHECKPOINT_DIR, args.exp_name + f'_final.pth'))
    model.load_state_dict(checkpoint['model'])
    final_clean_loss, final_clean_acc, _, _, _, final_pgd_loss, final_pgd_acc, final_pgd_delta_norm, _, _, _, _ = eval(args, model, testloader, criterion, finaleval=True)
