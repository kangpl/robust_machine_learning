...
def get_args():
    ...
    parser.add_argument('--percentile', default=85, type=float)
...
def train(args, model, trainloader, optimizer, criterion, step_lr_scheduler):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        multiple = 1.0 / np.percentile(abs(grad).cpu().numpy(), args.percentile)
        multi_grad = multiple * grad
        delta = clamp(delta + args.epsilon * multi_grad, -args.epsilon, args.epsilon)
        fgsm_delta = clamp(delta, lower_limit - inputs, upper_limit - inputs).detach()
